...
function M.config()
    ...
    map("n", prefix .. "bc", function()
        dap.toggle_breakpoint(ask("Condition"))
    end, { desc = "Toggle conditionally" })
    map("n", prefix .. "bh", function()
        dap.toggle_breakpoint(nil, ask("Hit number"))
    end, { desc = "Hit number" })
    map("n", prefix .. "bl", function()
        dap.toggle_breakpoint(nil, nil, ask("Message"))
    end, { desc = "Log" })
end
